...
public class UpdateBarGraph : MonoBehaviour {
	...
	public float xAxisMinValue = 16.0f;
...
//		Set (DateTime.Now);
	...
	void Set(DateTime day){
		...
		float maxLiter = 1000.0f;
		
		SaveWater s = new SaveWater ();
		int i = 0;
		for (DateTime d = day.AddDays(-14); d.Date <= day.Date; d = d.AddDays(1),i++) {
			...
			float L = s.GetDayWater (d);
			v[i] = L;
			maxLiter = (maxLiter < L ? L : maxLiter);
		}
		yAxisMaxValue = maxLiter;
