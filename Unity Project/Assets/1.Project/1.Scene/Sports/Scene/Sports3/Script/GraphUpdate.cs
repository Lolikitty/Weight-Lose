...
public class GraphUpdate: MonoBehaviour {
	...
	float[] v = {400.0f,1200.0f,1600.0f,2800.0f,2200.0f,2400.0f,2000.0f,2800.0f,4000.0f,800.0f,3200.0f,1200.0f,2000.0f,400.0f,1600.0f};
	...
	void InitGraphSet(){
		...
		barGraph.transform.localScale = new Vector2(2.0f,2.0f);
		barGraph.transform.localPosition = new Vector2 (-250.0f,-380.0f);
...
//		barGraph.GetComponent<WMG_Axis_Graph> ().xAxisLabelSize = 15;

		float maxKal = 1000.0f;
		SaveSports s = new SaveSports ();
		int i = 0;
		for (DateTime d = day.AddDays(-14); d.Date <= day.Date; d = d.AddDays(1),i++) {
			barGraph.GetComponent<WMG_Axis_Graph> ().xAxisLabels[i] = d.Day.ToString();
			float kal = s.GetDayKal (d);
			v[i] = kal;
			maxKal = (maxKal < kal ? kal : maxKal);
		}
		yAxisMaxValue = maxKal;
